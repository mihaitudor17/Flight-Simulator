...
void Airplane::ModifyYaw(const float& deltaTime, const int& dir)
{
	...
	yaw += deltaTime * -dir * 30.0f;
	rotation.z += deltaTime * dir * glm::radians(30.0f);
}

void Airplane::ModifyRoll(const float& deltaTime, const int& dir)
{
	roll += deltaTime * dir * rollIncrement;
}
...
void Airplane::Draw(const Shader* shader)
{
	...
	//model_mat = glm::rotate(model_mat, glm::radians(roll), {});
	...
	//auto rotation_mat = glm::yawPitchRoll(yaw, pitch, roll);
